...
如果机器只有 2 vCPU，建议把 RPC 业务线程池设为 `-DTHREAD_POOL_SIZE=1` 或 `-DTHREAD_POOL_SIZE=2`。默认值仍是 `4`，更适合资源稍充足的环境。
...
- 如果高并发压测出现 `recv error! errno:11`，通常是 RPC 客户端等待响应超时。可以降低 `-j` 并发、使用 `--rate` 限速逐步压测，或用 `-DRPC_CLIENT_TIMEOUT_MS=10000` 增大客户端超时时间。
